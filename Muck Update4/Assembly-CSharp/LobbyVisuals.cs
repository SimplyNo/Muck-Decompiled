...
public class LobbyVisuals : MonoBehaviour
{
  ...
  private void Awake()
  {
    ...
    for (int index = 0; index < this.lobbyPlayers.Length; ++index)
    {
      ...
      this.lobbyPlayerNames[index].gameObject.SetActive(false);
  ...
  public void OpenLobby(Lobby lobby)
  {
    ...
    foreach (OnlyActivateForHost lobbyPlayerName in this.lobbyPlayerNames)
      lobbyPlayerName.kickBtn.SetActive((long) SteamManager.Instance.PlayerSteamId.Value == (long) (ulong) lobby.Owner.Id);
  }
  ...
  public void SpawnLobbyPlayer(Friend friend)
  {
    ...
    this.lobbyPlayerNames[nextId].steamId = (SteamId) friend.Id.Value;
  }
