...
public class ClientHandle : MonoBehaviour
{
  ...
  public static void PickupItem(Packet packet)
  {
    ...
    if (!(bool) (UnityEngine.Object) component)
      return;
    if ((bool) (UnityEngine.Object) component.powerup)
  ...
  public static void PlayerHitObject(Packet packet)
  {
    ...
    Vector3 pos = packet.ReadVector3();
    switch ((PlayerStatus.WeaponHitType) packet.ReadInt())
    {
      case PlayerStatus.WeaponHitType.Undefined:
      case PlayerStatus.WeaponHitType.Rock:
      ...
      default:
        ...
        goto case PlayerStatus.WeaponHitType.Undefined;
    }
  }
  ...
  public static void PlayerHit(Packet packet)
  {
    ...
    MonoBehaviour.print((object) ("recevied player hit. Damage: " + (object) damage + ", ratio: " + (object) hpRatio + "from: " + (object) num + ", to: " + (object) key));
    PlayerManager player = GameManager.players[key];
    if (key == LocalClient.instance.myId)
      PlayerStatus.Instance.DealDamage(damage, 1, true, num);
    else
      player.SetHpRatio(hpRatio);
    player.hitable.Damage(damage, num, hitEffect, pos);
  }
  ...
  public static void RevivePlayer(Packet packet)
  {
    ...
    if (num1 == LocalClient.instance.myId && !flag)
    ...
    if (num1 != LocalClient.instance.myId)
      return;
  ...
  public static void PlayerDamageMob(Packet packet)
  {
    ...
    int hitEffect = packet.ReadInt();
    Vector3 pos = packet.ReadVector3();
    int killType = packet.ReadInt();
    HitableMob component = MobManager.Instance.mobs[key].GetComponent<HitableMob>();
    if (!(bool) (UnityEngine.Object) component)
      return;
    if (newHp <= 0 && LocalClient.instance.myId == fromClient)
      PlayerStatus.Instance.AddKill(killType, component.mob);
    int num = MobManager.Instance.mobs[key].hitable.Damage(newHp, fromClient, hitEffect, pos);
    PlayerStatus.WeaponHitType weaponHitType = (PlayerStatus.WeaponHitType) killType;
    switch (weaponHitType)
    {
      case PlayerStatus.WeaponHitType.Undefined:
        break;
      case PlayerStatus.WeaponHitType.Rock:
      ...
      default:
        if (num <= 0)
          break;
        GameManager.instance.onlyRock = false;
        Debug.LogError((object) ("hit: " + component.gameObject.name + ", type: " + (object) killType + ", type: " + (object) weaponHitType));
        break;
    }
  }
  ...
  public static void ReceiveStats(Packet packet) => GameManager.instance.MakeStats(packet);
}
