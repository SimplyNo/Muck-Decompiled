...
public class ServerHandle
{
  ...
  public static void KickPlayer(int client)
  {
    ...
    try
    {
      ServerSend.SendChatMessage(-1, "Server", Server.clients[client].player.username + " kicked");
    }
    catch
    {
      Debug.LogError((object) "Failed to send disconnect message to clients");
    }
    Server.clients[client] = (Client) null;
  }
  ...
  public static void PlayerHitObject(int fromClient, Packet packet)
  {
    ...
    Vector3 pos = packet.ReadVector3();
    int weaponHitType = packet.ReadInt();
    if (!ResourceManager.Instance.list.ContainsKey(num2))
      return;
    ...
    ServerSend.PlayerHitObject(fromClient, num2, num3, hitEffect, pos, weaponHitType);
    switch ((PlayerStatus.WeaponHitType) weaponHitType)
    {
  ...
  public static void PlayerHit(int fromClient, Packet packet)
  {
    ...
    Server.clients[fromClient].player.stats["DamageTaken"] += damage;
  }
  ...
  public static void PlayerDamageMob(int fromClient, Packet packet)
  {
    ...
    int num4 = component1.hp - damage;
    if (num4 > component1.maxHp)
      num4 = component1.maxHp;
    if (num4 <= 0)
    {
      num4 = 0;
    ...
    component1.hp = component1.Damage(num4, fromClient, hitEffect, pos);
    float knockbackMultiplier = PowerupInventory.Instance.GetKnockbackMultiplier(Server.clients[fromClient].player.powerups);
    if (((double) damage / (double) mob.hitable.maxHp > (double) mob.mobType.knockbackThreshold || (double) knockbackMultiplier > 0.0) && num4 > 0)
    {
    ...
    if (num4 <= 0 && LocalClient.instance.myId == fromClient)
      PlayerStatus.Instance.AddKill(num3, mob);
    ServerSend.PlayerHitMob(fromClient, num1, num4, hitEffect, pos, num3);
    switch ((PlayerStatus.WeaponHitType) num3)
    {
      case PlayerStatus.WeaponHitType.Undefined:
      case PlayerStatus.WeaponHitType.Rock:
        Server.clients[fromClient].player.stats["DamageDone"] += damage;
        break;
      default:
        if (num2 > 0)
        {
